...
// Validar parámetro
if (!isset($_GET['codigo_expediente'])) {
    jsonResponse(['error' => 'Parámetro "código de expediente" requerido'], 400);
}

$codigo_expediente = $_GET['codigo_expediente'];


// Consulta con filtro por código de expediente
$sql = <<<SQL
WITH base AS (
    ...
    WHERE ia.expcontratacio = :codigo_expediente
    )
    SELECT
      b.*,
...
try {
    ...
    $stmt->execute(['codigo_expediente' => $codigo_expediente]);
